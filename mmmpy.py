...
class MosaicTile:
    ...
    def __init__(
        ...
        # nc_path,wgrib2_path, wgrib2_name = (
        #     p if isinstance(p, Path) else Path(p)
        #     for p in (nc_path,wgrib2_path, wgrib2_name)
        # )
    ...
    def _get_tile_number(self):
        ...
        if self.Version == 1:
            ...
            if _are_equal(self.StartLat, 54.995) and _are_equal(
                self.StartLon, -129.995
            ):
                self.Tile = "1"
            elif _are_equal(self.StartLat, 54.995) and _are_equal(
                self.StartLon, -94.995
            ):
                self.Tile = "2"
            elif _are_equal(self.StartLat, 37.495) and _are_equal(
                self.StartLon, -129.995
            ):
                self.Tile = "3"
            elif _are_equal(self.StartLat, 37.495) and _are_equal(
    ...
    def _construct_header(self):
        ...
        Height = np.int32(
            self.Height * DEFAULT_Z_SCALE * ALTITUDE_SCALE_FACTOR
        ).tostring()  # km to m
...
class MosaicGrib(object):
    ...
    def format_grib_data(self):
        ...
        for i, gr in enumerate(self.gblist):
            ...
            if i == 0:
                ...
                self.Latitude = lat
                self.LatGridSpacing = np.round(
                    np.abs(self.Latitude[0, 0] - self.Latitude[1, 0]), decimals=2
                )
                self.LonGridSpacing = np.round(
                    np.abs(self.Longitude[0, 1] - self.Longitude[0, 0]), decimals=2
                )
                self.StartLat = np.max(self.Latitude)
                self.StartLon = np.min(self.Longitude)
                ...
                mrefl3d = np.zeros(
                    (
                        np.size(self.gblist),
                        np.shape(self.Latitude)[0],
                        np.shape(self.Longitude)[1],
                    ),
                    dtype="float",
                )
    ...
    def format_netcdf_data(self):
        ...
        self.LatGridSpacing = np.abs(
            self.nclist[0].latitude[0] - self.nclist[0].latitude[1]
        )
        self.LonGridSpacing = np.abs(
            self.nclist[0].longitude[0] - self.nclist[0].longitude[1]
        )
...
class MosaicStitch(MosaicTile):
    ...
    def help(self):
        ...
        print(
            "Instance = stitch_mosaic_tiles(map_array=map_array,",
            "direction=direction)",
        )
    ...
    def stitch_ns(self, n_tile=None, s_tile=None, verbose=False):
        ...
        if n_tile.nlon != s_tile.nlon:
            ...
            return
        inlat, index = self._stitch_radar_variables(
    ...
    def _stitch_metadata(self, a_tile=None, b_tile=None, index=None, ns_flag=True):
        ...
        """
        if ns_flag:
            self.Longitude = np.append(
                a_tile.Longitude, b_tile.Longitude[:index, :], axis=0
            )
            self.Latitude = np.append(
                a_tile.Latitude, b_tile.Latitude[:index, :], axis=0
            )
            if a_tile.Version == 1:
            ...
            self.Longitude = np.append(
                a_tile.Longitude[:, :index], b_tile.Longitude, axis=1
            )
            self.Latitude = np.append(
                a_tile.Latitude[:, :index], b_tile.Latitude, axis=1
            )
            if a_tile.Version == 1:
...
class MosaicDisplay(object):
    ...
    def plot_horiz(
        ...
        if basemap is None:
            m = self._create_basemap_instance(
                latrange, lonrange, resolution, area_thresh
            )
        else:
            m = basemap
        if embellish:
            m = self._add_gridlines_if_desired(
                m, parallels, meridians, linewidth, latrange, lonrange, show_grid
            )
    ...
    def plot_vert(
        ...
        vcut, xvar, xrange, xlabel, tlabel = self._get_vertical_slice(
            var, lat, lon, xrange, xlabel, verbose
        )
        if vcut is None:
            return
        # Plot details
    ...
    def _add_gridlines_if_desired(
        ...
        if show_grid:
            # Draw parallels
            vparallels = np.arange(
                np.floor(np.min(latrange)), np.ceil(np.max(latrange)), parallels
            )
            m.drawparallels(
                vparallels, labels=[1, 0, 0, 0], fontsize=10, linewidth=linewidth
            )
            # Draw meridians
            vmeridians = np.arange(
                np.floor(np.min(lonrange)), np.ceil(np.max(lonrange)), meridians
            )
            m.drawmeridians(
                vmeridians, labels=[0, 0, 0, 1], fontsize=10, linewidth=linewidth
            )
        return m
    ...
    def _get_vertical_slice(
        ...
        if lat is None and lon is None:
            ...
            if lat is None:
                ...
                if not xrange:
                    xrange = [
                        np.min(self.mosaic.Latitude),
                        np.max(self.mosaic.Latitude),
                    ]
                if not xlabel:
                    xlabel = "Latitude (deg)"
                vcut, xvar, tlabel = self._get_constant_longitude_cross_section(
                    var, lon
                )
    ...
    def _get_constant_latitude_cross_section(self, var=DEFAULT_VAR, lat=None):
        ...
        index = np.round(
            np.abs(lat - self.mosaic.StartLat) / self.mosaic.LatGridSpacing
        )
    ...
    def _get_constant_longitude_cross_section(self, var=DEFAULT_VAR, lon=None):
        ...
        index = np.round(
            np.abs(lon - self.mosaic.StartLon) / self.mosaic.LonGridSpacing
        )
        index = np.int32(index)
...
def stitch_mosaic_tiles(map_array=None, direction=None, verbose=False):
    ...
    # 2-D stitching in N-S and W-E
        ...
        if verbose:
            print(
                "Sent a 2-D matrix, attempting to stitch in both",
...
def _stitch_1d_array_we(
    ...
    for i in np.arange(np.shape(map_array)[0]):
        ...
        if i == 0:
            result.stitch_we(
                w_tile=map_array[i], e_tile=map_array[i + 1], verbose=verbose
            )
...
def _stitch_1d_array_ns(
    ...
    for i in np.arange(np.shape(map_array)[0]):
        ...
        if i == 0:
            result.stitch_ns(
                n_tile=map_array[i], s_tile=map_array[i + 1], verbose=verbose
            )
        if 1 <= i < np.shape(map_array)[0] - 1:
...
def _stitch_2d_array(map_array=None, verbose=False, method_name="stitch_mosaic_tiles"):
    ...
    for i in np.arange(np.shape(map_array)[1]):
        if not hasattr(map_array[0][i], DEFAULT_VAR) or not hasattr(
            map_array[1][i], DEFAULT_VAR
        ):
            _print_missing_a_tile(method_name)
            return False
        tmp = MosaicStitch()
        ns_stitches.append(tmp)
        ns_stitches[i].stitch_ns(
            n_tile=map_array[0][i], s_tile=map_array[1][i], verbose=verbose
        )
        if i == 0:
